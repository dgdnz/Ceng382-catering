...
export default function NewMenuItemPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess("Sweet creation saved successfully! 🎂");
      setTimeout(() => {
    ...
    <div className="min-h-screen bg-background p-6 md:p-12">
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-3xl p-8 sm:p-12 shadow-xl border border-primary border-opacity-30">
        <h1 className="text-3xl font-extrabold text-textDark mb-2 flex items-center gap-2">
          <span>🧁</span> Create New Pastry
        </h1>
        <p className="text-secondary font-medium mb-10">
          Build a dessert masterpiece, set optional customizations, and upload an enticing image.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-3 rounded-xl border border-primary border-opacity-60 text-textDark bg-background bg-opacity-30 focus:outline-none focus:ring-secondary focus:border-secondary sm:text-sm"
                placeholder="Briefly describe the cake/pastry, ingredients, texture, or allergy warnings..."
                value={description}
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <div className="flex-1 w-full">
                <label className="block text-sm font-bold text-textDark mb-1">Upload File (Mandatory)</label>
          ...
          <div className="space-y-6">
            ...
            {groups.length > 0 ? (
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-dashed border-primary border-opacity-20">
                      <div className="flex-1">
                        <input
                          ...
                          placeholder="Group Name (e.g. Size, Removable Ingredients)"
                          value={group.name}
                      ...
                      <button
                        ...
                        className="text-red-500 hover:text-red-700 bg-white p-2 rounded-full border border-red-200 shadow-sm transition duration-150Self self-end md:self-auto"
                      >
                        <Trash className="w-4 h-4" />
                      </button>
                    </div>

                    {/* Option constructor nested list */}
                            ...
                            <div key={optIdx} className="grid grid-cols-1 md:grid-cols-5 gap-3 items-center bg-white p-2.5 rounded-xl border border-primary border-opacity-20">
                              <div className="col-span-3">
                                <input
                                  ...
                                  placeholder="e.g. Extra Raspberry Syrup, Large Size, No Pecans"
                                  value={option.name}
              ...
              <div className="text-center py-8 bg-background bg-opacity-10 border-2 border-dashed border-primary border-opacity-30 rounded-2xl">
                ...
                <p className="text-xs text-textLight mt-1">Add customization groups so clients can choose sizes, select removals or add extras.</p>
              </div>
            )}
          </div>
          ...
          <div className="pt-6 border-t border-primary border-opacity-20 flex justify-end">
            ...
            >
              <Save className="w-5 h-5" /> {loading ? "Saving Sweet Pastry..." : "Save Menu Item 🍰"}
            </button>
          </div>
